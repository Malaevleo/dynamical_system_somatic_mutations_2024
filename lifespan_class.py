...
class Somatic_LS(object):
    ...
    def calculate_population(self):
        ...
        solution = solve_ivp(model,t_span=(self.start, self.end), y0=initial, t_eval=self.t, method = self.method,args=conf, dense_output=False, atol = 1e-6, rtol = 1e-6)
        self.sol = solution
        return solution  
    ...
    def _plot_mortality(self, arr, life:int) -> None:
        ...
        plt.plot(self.t[:-1]*21.5 / (365.25*24), deriv)
        plt.xlabel('Years')
        plt.ylabel('M(t)')
        plt.title('Mortality function')
        plt.axvline(maximum * 21.5/(365.25 * 24), color = 'r', ls = '--')
    ...
    def _variator(self, fraction, sampling_freq, model, config, init, x_bound, legend, thr, z_min, z_max, d_min, d_max, K, proportion):
        ...
        for i in range(len(axs)):
            for j in range(len(sols[i])):
                axs[i].plot(self.t*21.5/(365.25*24), sols[i][j].y[0] if not proportion else sols[i][j].y[0]/K, label = f'Somatic population for {names[param_ind[i]]} = {np.round(ranges[i][j], mantissa[param_ind[i]])}')
            axs[i].grid('True')
