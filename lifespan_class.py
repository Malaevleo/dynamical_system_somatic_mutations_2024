...
class Somatic_LS(object):
    ...
    def custom_params(self, uploaded=None) -> Tuple[dict,...]:
        ...
        if type(uploaded) == str:
            ...
            custom_conf = self._clear_dict(custom_conf)
        elif type(uploaded) == dict:
            custom_conf = self._clear_dict(uploaded)
            params = custom_conf['parameters']
            init = custom_conf['initial conditions']
            thr = custom_conf['threshold']
            return params, init, thr
        elif type(uploaded) == pd.core.frame.DataFrame:
            custom_conf = uploaded.to_dict()
            custom_conf = self._clear_dict()
        else:
    ...
    def lifespan(
        ...
        if len(l) != 0:
            if self.organ == 'mouse liver':
                print('Life expentancy (years):', np.round(l[0] * self.coeff, 2))
            else:
                print('Life expentancy (years):', np.round(l[0] * self.coeff))
            print('-'*50)
            ls = l[0]
        else:
            print('Havent died')
    ...
    def _plot_mortality(self, arr, life, derivative, logder) -> None:
        ...
        if not derivative:
            ...
            print('Max deriv value:', np.round(max_, 0))
            print('Max deriv moment:', np.round(max_der_*self.coeff, 0))
            if type(life)==int:
                print('Ratio of max deriv to total lifespan in %:', np.round((max_der_ * self.coeff * 100)/(life * self.coeff), 1))
    ...
    def _plot_stable_points(self, root, proportions, plot_thr) -> None:
        ...
        for i in range(len(t)):
            if root == 1:
                res[i] = 0.5*(K*a**2*b*t[i]**2 + 2.0*r)/(a**2*b*t[i]**2) - 1.4142135623731*np.sqrt(0.125*K**2*a**4*b**2*t[i]**4 + K*a**3*b*t[i]**2 - 0.5*K*a**2*b*r*t[i]**2 + 0.5*r**2)/(a**2*b*t[i]**2)
            elif root == 2:
                res[i] = 0.5*(K*a**2*b*t[i]**2 + 2.0*r)/(a**2*b*t[i]**2) + 1.4142135623731*np.sqrt(0.125*K**2*a**4*b**2*t[i]**4 + K*a**3*b*t[i]**2 - 0.5*K*a**2*b*r*t[i]**2 + 0.5*r**2)/(a**2*b*t[i]**2)
    ...
    def variator(
        ...
        if only_r:
            self._vary_parameter('r', fraction, sampling_freq, model, init, thr, x_bound, legend,config_,K, proportions, minimum, maximum)
        elif only_alpha:
            self._vary_parameter('alpha', fraction, sampling_freq, model, init, thr, x_bound, legend, config_, K, proportions, minimum, maximum)
        elif only_sigma:
            self._vary_parameter('sigma', fraction, sampling_freq, model, init, thr, x_bound, legend, config_, K, proportions, minimum, maximum)
        elif only_z:
    ...
    def _plot_variation(self, solutions, param_range, param_name, threshold, x_bound, legend, K, proportion):
        ...
        for solution, param_value in zip(solutions, param_range):
            ax.plot(self.t * self.coeff, solution.y[0] if not proportion else solution.y[0]/K, label=f'{param_name} = {np.round(param_value, 13)}')
